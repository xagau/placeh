placeh Core version 0.12.0 is now available from:

  <https://placeh.org/bin/placeh-core-0.12.0/>
...
Previous versions of placeh Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of placeh Core was to raise the default minimum
relay fee.

placeh Core 0.12 will have a strict maximum size on the mempool. The
...
placeh Core 0.12 also introduces new default policy limits on the length and
...
placeh Core 0.12 nodes. placeh Core will only allow replacement of
...
Note that the wallet in placeh Core 0.12 does not yet have support for
...
This allows running placehd without having to do any manual configuration.
...
placeh Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. placeh Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In placeh Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in placeh Core 0.13, and it is at this time undecided whether the
...
placeh Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
placeh Core automatically creates a hidden service to listen on, without
manual configuration. placeh Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if placeh Core is listening, and
...
placehd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals placeh
Core to use floating fees. By default, placeh Core will use floating
fees.
...
(default: `0.0002` PHL/kB).

At all times, placeh Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PHL.
Furthermore, placeh Core will never create transactions paying less than
...
`placeh.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /placehrpc>
        ...
        # optional bypass placehd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< placehrpc:<password>
    </Location>
...
Restarting placehd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of placeh-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new placeh-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include placeh-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix placeh-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of placeh-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override PLACEHD and PLACEHCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [placeh-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add PLACEHD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libplacehconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "PHL" (Ross Nicoll)
