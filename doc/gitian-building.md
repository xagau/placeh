...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-placeh-host/README.md).
...
Clone the git repositories for placeh and Gitian.
...
in the placeh repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/placeh/.git/
...
For example:
```bash
URL=https://github.com/laanwj/placeh.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit placeh=${COMMIT} --url placeh=${URL} ../placeh/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit placeh=${COMMIT} --url placeh=${URL} ../placeh/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit placeh=${COMMIT} --url placeh=${URL} ../placeh/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the placeh git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../placeh/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url placeh=${PHLPATH},signature=${SIGPATH} ../placeh/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/placeh-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/placeh-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/placeh-osx-build.assert
```
