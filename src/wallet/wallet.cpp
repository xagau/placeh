...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE) {
        std::string strAddr = CplacehAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/placeh/placeh/pull/10340, but that change has been
    // postponed until after https://github.com/placeh/placeh/pull/10244 to
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CplacehAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CplacehAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CplacehAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CplacehAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CplacehAddress(address).ToString());
}
 ...
 * https://github.com/placeh/placeh/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination& dest, const std::string& key, const std::string& value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CplacehAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination& dest, const std::string& key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CplacehAddress(dest).ToString(), key);
}
