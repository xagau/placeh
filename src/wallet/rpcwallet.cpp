...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    return CplacehAddress(keyID).ToString();
}


CplacehAddress GetAccountAddress(CWallet* const pwallet, std::string strAccount, bool bForceNew = false)
{
    CPubKey pubKey;
    ...
    return CplacehAddress(pubKey.GetID());
}
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    CplacehAddress address(request.params[0].get_str());
    if (!address.IsValid())
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    for (const std::pair<CplacehAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        const CplacehAddress& address = item.first;
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    for (std::set<CTxDestination> grouping : pwallet->GetAddressGroupings()) {
        ...
        for (CTxDestination address : grouping) {
            ...
            addressInfo.push_back(CplacehAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwallet->mapAddressBook.find(CplacehAddress(address).Get()) != pwallet->mapAddressBook.end()) {
                    addressInfo.push_back(pwallet->mapAddressBook.find(CplacehAddress(address).Get())->second.name);
                }
            }
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    CplacehAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    // placeh address
    CplacehAddress address = CplacehAddress(request.params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid placeh address");
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    std::string strAccount = AccountFromValue(request.params[0]);
    CplacehAddress address(request.params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid placeh address");
    CAmount nAmount = AmountFromValue(request.params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase(pwallet) + "\n"
                                               ...
                                               "    {\n"
                                               "      \"address\":amount   (numeric or string) The placeh address is the key, the numeric amount (can be string) in " +
    ...
    std::set<CplacehAddress> setAddress;
    std::vector<CRecipient> vecSend;

    CAmount totalAmount = 0;
    std::vector<std::string> keys = sendTo.getKeys();
    for (const std::string& name_ : keys) {
        CplacehAddress address(name_);
        if (!address.IsValid())
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    return CplacehAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    return CplacehAddress(w.result).ToString();
}
...
UniValue ListReceived(CWallet* const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map<CplacehAddress, tallyitem> mapTally;
    for (const std::pair<uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        const CWalletTx& wtx = pairWtx.second;
    ...
    for (const std::pair<CplacehAddress, CAddressBookData>& item : pwallet->mapAddressBook) {
        ...
        std::map<CplacehAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CplacehAddress address(input.get_str());
            if (!address.IsValid())
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CplacehAddress(address).ToString()));
    ...
    if (!request.params[1].isNull()) {
        if (request.params[1].type() == UniValue::VBOOL) {
            ...
            if (options.exists("changeAddress")) {
                CplacehAddress address(options["changeAddress"].get_str());
