    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.startsWith(PLACEH_IPC_PREFIX, Qt::CaseInsensitive)) // placeh: URI
        {
            ...
            if (GUIUtil::parseplacehURI(arg, &r) && !r.address.isEmpty()) {
                CplacehAddress address(r.address.toStdString());
    ...
    if (s.startsWith(PLACEH_IPC_PREFIX, Qt::CaseInsensitive)) // placeh: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseplacehURI(s, &recipient)) {
                CplacehAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CplacehAddress(dest).ToString()));
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).").arg(placehUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
