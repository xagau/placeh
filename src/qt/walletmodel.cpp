...
{
    CplacehAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient& rcp : recipients) {
        ...
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered placeh address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CplacehAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
    ...
    for (const SendCoinsRecipient& rcp : transaction.getRecipients()) {
        ...
        if (!rcp.paymentRequest.IsInitialized()) {
            ...
            CTxDestination dest = CplacehAddress(strAddress).Get();
...
{
    QString strAddress = QString::fromStdString(CplacehAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput>>& mapCoins) const
{
    for (auto& group : wallet->ListCoins()) {
        auto& resultGroup = mapCoins[QString::fromStdString(CplacehAddress(group.first).ToString())];
...
{
    CTxDestination dest = CplacehAddress(sAddress).Get();
