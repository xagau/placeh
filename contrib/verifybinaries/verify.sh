...
###   placeh.io and placeh.io and compares them.
...
WORKINGDIR="/tmp/placeh_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://placeh.io"
HOST2="https://placeh.io"
BASEDIR="/bin/"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'placeh-',
...
if [ $? -ne 0 ]; then
   echo "placeh.io failed to provide signature file, but placeh.io did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "placeh.io and placeh.io signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the placeh core binary release signing key installed?"
   fi
